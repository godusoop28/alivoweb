...
import { AppointmentAccess } from "@/lib/api/types";
import { ApiError } from "@/lib/api/client";

export default function AdminAppointmentAccess() {
  const [accesses, setAccesses] = useState<AppointmentAccess[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({ email: "", reason: "", expiresAt: "" });
  ...
  const load = () => {
    setLoading(true);
    adminApi
      .listAppointmentAccess()
      .then(({ accesses }) => setAccesses(accesses))
  ...
  const handleGrant = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await adminApi.grantAppointmentAccess({
        email: form.email,
        reason: form.reason || undefined,
        expiresAt: form.expiresAt || undefined,
      });
      setForm({ email: "", reason: "", expiresAt: "" });
      setSuccess(true);
  ...
  const handleRevoke = async (id: string) => {
    try {
      await adminApi.revokeAppointmentAccess(id);
      load();
    } catch {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-alivos-dark">Citas sin costo</h1>
        <p className="text-slate-500 text-sm mt-1">
          Da acceso a un usuario para que pueda agendar citas de asesoramiento sin pagar
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-brand-200 rounded-2xl p-5 flex gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600 leading-relaxed">
            Mientras el usuario tenga un acceso activo aquí, todas las citas que agende quedan{" "}
            <strong>sin costo</strong> — no se le pedirá pagar en Mercado Pago. Útil para terapeutas
            colaboradoras, cortesías o casos especiales. Puedes quitarle el acceso cuando quieras.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <h2 className="font-bold text-alivos-dark mb-5">Dar acceso a un usuario</h2>
        {success && (
        ...
        <form onSubmit={handleGrant} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-300"
              placeholder="Ej: Terapeuta colaboradora, cortesía..."
            />
          </div>
          <button
            type="submit"
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="font-bold text-alivos-dark">Accesos recientes</h2>
                ...
                {accesses.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-5 py-10 text-center text-slate-400 text-sm">
                      Todavía no se otorgaron accesos.
                    </td>
                  </tr>
                )}
              </tbody>
