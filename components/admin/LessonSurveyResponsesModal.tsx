...
import { FormField, LessonSurveyResponse } from "@/lib/api/types";
import Modal from "@/components/ui/Modal";

interface LessonSurveyResponsesModalProps {
  attachmentId: string;
  title: string;
  fields: FormField[];
  onClose: () => void;
...
export default function LessonSurveyResponsesModal({ attachmentId, title, fields, onClose }: LessonSurveyResponsesModalProps) {
  const [responses, setResponses] = useState<LessonSurveyResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    adminApi
      .listLessonSurveyResponses(attachmentId)
  ...
  return (
    <Modal title={`Respuestas — ${title}`} onClose={onClose} maxWidth="max-w-2xl">
      {loading ? (
        <p className="text-sm text-slate-400">Cargando...</p>
      ) : error ? (
        <p className="text-sm text-red-600">{error}</p>
      ) : responses.length === 0 ? (
        <p className="text-sm text-slate-400">Todavía nadie ha respondido esta encuesta.</p>
      ) : (
