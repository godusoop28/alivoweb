...
import { getLessonSurveyResponse, submitLessonSurveyResponse } from "@/lib/api/courses";
import { FormField } from "@/lib/api/types";
import { ApiError } from "@/lib/api/client";

interface LessonSurveyRendererProps {
  attachmentId: string;
  title: string;
  fields: FormField[];
}
...
export default function LessonSurveyRenderer({ attachmentId, title, fields }: LessonSurveyRendererProps) {
  ...
  useEffect(() => {
    ...
    setAnswers({});
    setSubmittedAt(null);
    getLessonSurveyResponse(attachmentId)
      .then((response) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const response = await submitLessonSurveyResponse(attachmentId, JSON.stringify(answers));
      setSubmittedAt(response.submittedAt);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "No se pudo enviar la encuesta.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className="text-sm text-slate-400 py-2">Cargando encuesta...</div>;
  }

  return (
    <div>
      <h4 className="text-sm font-semibold text-alivos-dark mb-2">{title}</h4>
      {submittedAt && (
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          {saving ? "Enviando..." : submittedAt ? "Actualizar respuestas" : "Enviar encuesta"}
        </button>
      </form>
    </div>
  );
